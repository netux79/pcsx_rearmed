...
static PadDataS pad[PORTS];
static int reqPos, respSize, req;
static int ledStateReq44[PORTS];
...
unsigned char _PADpoll(int port, unsigned char value) {
	...
	switch(reqPos){
		...
		case 3:
            ...
            // vibration for analog controller
            if (pad[port].controllerType == PSE_PAD_TYPE_ANALOGPAD)
               vibrate(port);
				break;
			}
		break;
...
unsigned char CALLBACK PAD1__startPoll(int padIndex) {
	reqPos = 0;
	PAD1_readPort1(&pad[0]);
	_PADstartPoll(&pad[0]);
	return 0x00;
}

unsigned char CALLBACK PAD1__poll(unsigned char value) {
	return _PADpoll(0, value);
}
...
unsigned char CALLBACK PAD2__startPoll(int padIndex) {
	reqPos = 0;
	PAD2_readPort2(&pad[1]);
	_PADstartPoll(&pad[1]);
	return 0x00;
}
